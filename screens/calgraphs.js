...
const calgraphs = ({navigation}) => {
  ...
  saveData = async () => {
    ...
    const fetchAPI2 = async () => {
        return await fetch('http://192.168.143.98:3000/limit/adduser1', requestOptions)
    ...
    const readDataMongo = async () => {
      ...
      //  console.log("mongo")
       ...
       const fetch2 = async () => {
        const uri = `http://192.168.143.98:3000/limit/calorie/${user.uid}`
        return await fetch(uri)
        .then((response) => response.json())
        .then((result) => {
         console.log(result)
    ...
    return(
        <Layout style= {styles.container}>
            ...
            <View style= {styles.row}>         
               <Button style={styles.button} status='primary'  onPress= {() => navigation.navigate("Calorie")}>
      Add Record
    </Button>

    <Button style={styles.button} status='primary' onPress= {() => navigation.navigate("calrecord")}>
      View All Records
    </Button>
